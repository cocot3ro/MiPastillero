...
class CalendarRecyclerViewAdapter(
    ...
    override fun getItemCount(): Int {
        return list.map { it.horario }.flatten().distinctBy { it.time }.sortedBy { it.time }.size
    }

    override fun onBindViewHolder(holder: CalendarPageViewHolder, position: Int) {
        val hora = list.map { it.horario }.flatten().distinctBy { it.time }
            .sortedBy { it.time }[position]
